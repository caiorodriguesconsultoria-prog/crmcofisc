...
type AndamentoHistorico = { id: string; texto: string; data: string; incluir_relatorio: boolean };
...
export default function HistoricoLazy({ processoId }: { processoId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase
        .from("andamentos")
        .select("id, texto, data, incluir_relatorio")
        .eq("processo_id", processoId)
        .neq("tipo", "Tarefa concluída")
        .order("data", { ascending: false }),
    ]).then(([{ data: kanban }, { data: eventos }, { data: tarefasConcluidas }, { data: andamentos }]) => {
      if (ativo)
        setDados({
  ...
  return (
    <>
      ...
      <div style={{ marginTop: 14 }}>
        <strong style={{ fontSize: 12.5 }}>Andamentos</strong>
        <ul style={{ margin: "6px 0 0", paddingLeft: 20, fontSize: 13 }}>
          {dados.andamentos.length === 0 && (
            <li style={{ color: cor.textoTerciario }}>Nenhum andamento registrado ainda.</li>
          )}
          {dados.andamentos.map((a) => (
            <li key={a.id}>
              {a.texto} — {new Date(a.data).toLocaleString("pt-BR")}
              {a.incluir_relatorio ? " (Ocorrência)" : ""}
            </li>
          ))}
        </ul>
      </div>
