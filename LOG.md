...
## 2026-08-25 · Pendência levantada — "pessoas de coordenação" (contatos de áreas técnicas)
Caio mencionou um cadastro diferente de Gestor/Fiscal: pessoas de "coordenações" (áreas técnicas que avaliam eventos/ocorrências), com nome, e-mail, ramal e a coordenação (nome + sigla) a que pertencem — sem matrícula — e cada campo com botão de copiar (pra facilitar encaminhar e-mail). Não é o mesmo conceito da tabela `coordenacoes` atual (CGCEAF/CGAFME/CGAFB, usadas hoje só pra classificar processo e, até esta mudança, gestor/fiscal). Ainda não implementado — falta confirmar com Caio se são contatos dentro dessas mesmas 3 coordenações ou de um conjunto diferente/mais amplo de Coordenações-Gerais externas, antes de desenhar a tabela nova.
